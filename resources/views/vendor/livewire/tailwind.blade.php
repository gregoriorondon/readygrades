            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm leading-5">
                        <span class="font-inter">{!! ucwords('mostrando registros del') !!}</span>
                        <span class="font-inter">{{ $paginator->firstItem() }}</span>
                        <span class="font-inter">{!! ucwords('al') !!}</span>
                        <span class="font-inter">{{ $paginator->lastItem() }}</span>
                        {{-- <span class="font-inter">{!! ucwords('de un total de') !!}</span> --}}
                        {{-- <span class="font-inter">{{ $paginator->total() }}</span> --}}
                        {{-- <span class="font-inter">{!! ucwords('registros') !!}</span> --}}
                    </p>
                </div>
                        ...
                        <span>
                            ...
                            @if ($paginator->onFirstPage())
                                <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                                    <span class="relative inline-flex items-center px-2 py-2 text-sm text-gray-500 border border-gray-300 cursor-default font-inter rounded-l-md leading-5" aria-hidden="true">
                        ...
                        @foreach ($elements as $element)
                            ...
                            @if (is_string($element))
                                <span aria-disabled="true">
                                    <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm border border-gray-300 cursor-default font-inter leading-5">{{ $element }}</span>
                                </span>
                            @endif
                                    ...
                                    <span wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}">
                                        @if ($page == $paginator->currentPage())
                                            <span aria-current="page">
                                                <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm border border-gray-300 cursor-default font-inter bg-gray-400/20 leading-5">{{ $page }}</span>
                                            </span>
                                        @else
                                            <button type="button" wire:click="gotoPage({{ $page }}, '{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm text-gray-700 border border-gray-300 font-inter leading-5 hover:text-gray-500 focus:z-10 focus:outline-none focus:border-blue-300 focus:ring ring-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                                {{ $page }}
                                            </button>
                                        @endif
                                    </span>
                        ...
                        <span>
                            ...
                            @else
                                <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                                    <span class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium border border-gray-300 cursor-default rounded-r-md leading-5" aria-hidden="true">
                                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                                        </svg>
                                    </span>
